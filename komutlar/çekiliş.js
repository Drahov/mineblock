...
if(!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`:flag_tr:  *Gerekli yetkiye sahip değilsin*`)
if(!args[0]) return message.channel.send(`:flag_tr:  *Bir argüman girmelisin: +çekiliş başlat/tekrar*`)
let arg = ['başlat', 'tekrar']
if(!arg.includes(args[0])) return message.channel.send(`:flag_tr:  *Sadece +çekiliş başlat/tekrar kullanabilirsin.*`)
...
message.channel.send(`:flag_tr: *Çekiliş hangi kanalda yapılacak?*`).then(() => {
message.channel.awaitMessages(filter, { maxMatches: 1, time: 60000, errors: ['time'] }).then(collected => {
let as = collected.first().content.replace('<#', '').replace('>', '')
let ch = message.guild.channels.get(as)
if(!ch) return message.channel.send(`*:flag_tr:  Etiketlediğin kanalı bulamadım, işlem iptal edildi.*`)
message.delete()
message.channel.send(`Kanal: ${ch}`).then(() => kanal.push(ch.id))
  
message.channel.send(`*:flag_tr:  Çekiliş süresi ne kadar? (1 dakika/1 saat)*`).then(() => {
message.channel.awaitMessages(filter, { maxMatches: 1, time: 60000, errors: ['time'] }).then(collected => {
...
}).catch(collected => { message.channel.send(`:flag_tr:  *Çekiliş süresi girmediğin için iptal edildi.*`); });
})
}).catch(collected => { message.channel.send(`:flag_tr:  *Çekiliş kanalı girmediğin için iptal edildi.*`); });
})
}).catch(collected => { message.channel.send(`:flag_tr:  *Çekiliş ödülü girmediğin için iptal edildi.*`); });
})
...
if(!args[1]) return message.channel.send(`:flag_tr:  *Çekilişin yapıldığı kanalı etiketle.*`)
if(!channel) return message.channel.send(`:flag_tr:  *Etiktlediğin kanalı bulamıyorum.*`)

let mesaj = args[2]
if(!mesaj) return message.channel.send(`:flag_tr:  *Bir mesaj ID'si girmeyi unuttun.*`)
if(isNaN(mesaj)) return message.channel.send(`:flag_tr:  *Bir mesaj ID'si girmelisin.*`)
...
if(!ads) return message.channel.send(`:flag_tr:  *Hala bitmemiş olan veya çekiliş mesajı olmayan bir mesajın ID'sini girdin.*`)
